...
<head>
					...
					success : function(data, status) {
						...
						$.map(data, function(item, index){
							...
							for(var i = 0 ; i < item.length ; i++) {
								...
								$(".ct_list_pop:eq(" + i + ") td:eq(2)").text(item[i].prodName);
								$(".ct_list_pop:eq(" + i + ") td:eq(4)").text(item[i].price);
								$(".ct_list_pop:eq(" + i + ") td:eq(6)").text(item[i].prodDetail);
								
								if(item[i].prodQuantity == 0) {
									$(".ct_list_pop:eq(" + i +") td:eq(8)").text("재고없음_");
								} else {
									$(".ct_list_pop:eq(" + i +") td:eq(8)").text("판매중_");
								}
							}
	...
	$(function(){
		...
		$(".ct_list_pop td:nth-child(5)").hover(function(){
